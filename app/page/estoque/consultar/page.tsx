            ...
            <div className="mx-auto max-w-4xl space-y-6">
                ...
                <div>
                    ...
                    <p className="mt-1 text-sm text-[#5A6469]">
                        ...
                        use{" "}
                        <Link href="/page/estoque" className="font-semibold text-[#0086FF] hover:underline">
                            Estoque
                        </Link>.
                    </p>
                </div>
                ...
                <section className="card p-5 sm:p-6">
                    ...
                    <form onSubmit={consultarCodigo} className="space-y-1.5">
                        ...
                        <div className="flex gap-2">
                            <div className="relative flex-1">
                                <FiTag className="absolute left-3 top-1/2 w-4 -translate-y-1/2 text-[#8C969B]" aria-hidden />
                                <input
                                    id="codigo"
                                    type="text"
                                    value={codigo}
                    ...
                    {erroCodigo && (
                        <p role="alert" className="mt-4 rounded-lg bg-[#FDECEA] px-4 py-3 text-sm font-semibold text-[#D4351C]">
                            {erroCodigo}
                        </p>
                    )}

                    {consulta && (
                        <div className="mt-5 border-t border-[#E4E9EB] pt-5">
                            ...
                            <div className="mt-4 rounded-lg bg-[#E6F3FF] p-4">

                                <p className="text-xs font-bold uppercase tracking-[0.08em] text-[#0075E2]">
                                    Onde está
                                </p>
                                        ...
                                        {Object.entries(consulta.locais).map(([local, quantidade]) => {
                                            ...
                                            return (
                                                <li key={local} className="flex items-center justify-between gap-3">
                                                    <span className="flex items-center gap-2 font-display text-lg text-[#1E2428]">
                                                        <FiMapPin className="w-4 shrink-0 text-[#0075E2]" aria-hidden />
                                                        Rua {rua} · Bloco {bloco}
                                                    </span>
                            ...
                            <dl className="mt-4 grid grid-cols-2 gap-y-2 text-sm sm:grid-cols-4">
                                <dt className="text-[#5A6469]">Disponíveis</dt>
                                <dd className="num font-bold text-[#1E2428]">{consulta.resumo.disponiveis}</dd>
                                <dt className="text-[#5A6469]">Reservadas</dt>
                                <dd className="num font-bold text-[#1E2428]">{consulta.resumo.reservadas}</dd>
                                <dt className="text-[#5A6469]">Vendidas</dt>
                                <dd className="num font-bold text-[#1E2428]">{consulta.resumo.vendidas}</dd>
                                <dt className="text-[#5A6469]">Avariadas</dt>
                                <dd className="num font-bold text-[#1E2428]">{consulta.resumo.avariadas}</dd>
                            </dl>

                            {/* DE ONDE VEIO */}
                            {consulta.procedencia && (
                                <div className="mt-4 border-t border-[#E4E9EB] pt-4">

                                    <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.08em] text-[#5A6469]">
                                        <FiTruck className="w-3.5" aria-hidden />
                                        De onde veio
                                    </p>

                                    <p className="mt-1.5 text-sm text-[#1E2428]">
                ...
                {termo && (
                    <section className="space-y-3">
                        {achados.length === 0 && (
                            <p className="card p-6 text-center text-sm text-[#5A6469]">
                                        ...
                                        <ul className="mt-3 space-y-1.5">
                                            {[...porLocal.entries()].map(([local, quantidade]) => (
                                                <li key={local} className="flex items-center justify-between gap-3 text-sm">
                                                    <span className="flex items-center gap-2 text-[#1E2428]">
                    ...
                    {locais.map((local) => (
                        <article key={local.codigo || "sem-local"} className="card p-5">
                            ...
                            <div className="flex items-center justify-between gap-3">
                                ...
                                <span className="num text-sm font-bold text-[#5A6469]">
                                    {local.total} peça(s)
                                </span>

                            </div>

                            <ul className="mt-3 space-y-1.5 border-t border-[#E4E9EB] pt-3">
                                    ...
                                    return (
                                        <li key={produtoId} className="flex items-center justify-between gap-3 text-sm">
                                            ...
                                            <span className="min-w-0 truncate text-[#1E2428]">
                                                ...
                                                {variacao ? (
                                                    <span className="text-[#5A6469]"> · {variacao}</span>
                                                ) : null}
                                            </span>

                                            <span className="num shrink-0 font-bold text-[#1E2428]">
                                                {quantidade}
                                            </span>
